...
export function renderProjectBody(
  ...
  return `<div class="detail-grid" data-project-body><div class="detail-identity">${mainImage}
    ...
    ${options.privatePreview ? '<a class="text-link" href="/participants/">Participant help ↗</a>' : '<a class="text-link" href="/visit/">Visit information ↗</a>'}</aside><div class="detail-reading">${reading}</div></div>`;
}
