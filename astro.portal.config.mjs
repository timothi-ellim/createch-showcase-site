...
if (
  process.argv[process.argv.indexOf('--mode') + 1] !== 'portal-staging' ||
  ...
  throw new Error(
    'PORTAL STAGING BUILD BLOCKED: exact hosted API, staging environment and publishable key required.',
  );
}
      ...
      hooks: {
        'astro:config:setup': ({ injectRoute }) => {
          for (const [pattern, entrypoint] of [
            ['/motion.css', './src/lib/motion/style.ts'],
            [
              '/participant/[...path]',
              './src/pages/participant/[...path].astro',
            ],
